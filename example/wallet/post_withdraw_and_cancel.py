from huobi.client import WalletClient
from huobi.constant import *


# case 1: TRX withdraw and cancel
wallet_client = WalletClient(api_key=g_api_key, secret_key=g_secret_key, url=HUOBI_URL_VN)
withdraw_id_new = wallet_client.post_create_withdraw(address="xxxxxx",
                                                     amount=40, currency="trx", fee=1,
                                                     chain=None, address_tag=None)

withdraw_id_formated = withdraw_id_new*100 + 1
print("Create Withdraw ID ", withdraw_id_new)

withdraw_id_ret = wallet_client.post_cancel_withdraw(withdraw_id=withdraw_id_formated)
print("Cancel Withdraw ", withdraw_id_ret)


# case 2: USDT withdraw and cancel
wallet_client = WalletClient(api_key=g_api_key, secret_key=g_secret_key, url=HUOBI_URL_VN)
withdraw_id_new = wallet_client.post_create_withdraw(address="xxxxxxxx",
                                                     amount=2, currency="usdt", fee=1,
                                                     chain="usdterc20", address_tag=None)

withdraw_id_formated = withdraw_id_new*100 + 1
print("Create Withdraw ID ", withdraw_id_new)

withdraw_id_ret = wallet_client.post_cancel_withdraw(withdraw_id=withdraw_id_formated)
print("Cancel Withdraw ", withdraw_id_ret)



